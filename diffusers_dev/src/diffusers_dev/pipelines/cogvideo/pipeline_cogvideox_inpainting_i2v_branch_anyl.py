...
class CogVideoXI2VDualInpaintAnyLPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __init__(
        ...
        self.gain_video_processor = VideoProcessor(vae_scale_factor=self.vae_scale_factor_spatial, do_normalize=False, do_binarize=False, do_convert_grayscale=True)
    ...
    def __call__(
        ...
        ori_video: List[PIL.Image.Image] = None,
        ref_video: List[PIL.Image.Image] = None,
        ...
        for window_idx in range(n_windows):
            ...
            if ori_video is not None:
                if isinstance(ori_video, (list, tuple)):
                    window_ori_video = ori_video[start_frame:end_frame]
                else:
                    window_ori_video = ori_video[:, start_frame:end_frame]
                window_ori_video = self.video_processor.preprocess_video(window_ori_video, height=height, width=width)
                window_ori_video = window_ori_video.to(dtype=torch.float32)
            else:
                window_ori_video = None

            if isinstance(ref_video, (list, tuple)):
                window_ref_video = ref_video[start_frame:end_frame]
            else:
                window_ref_video = ref_video[:, start_frame:end_frame]
            window_ref_video = self.video_processor.preprocess_video(window_ref_video, height=height, width=width)
            window_ref_video = window_ref_video.to(dtype=torch.float32)
            ...
            mask_condition = self.gain_video_processor.preprocess_video(window_mask, height=height, width=width)
            if masked_video_latents is not None:
                masked_video_latents = None
            
            if masked_video_latents is None:
                if window_ori_video is not None:
                    masked_video = window_ori_video
                else:
                    masked_video = window_video
            else:
            ...
            # prepare ref latents
            window_ref_video = window_ref_video.to(device=device, dtype=prompt_embeds.dtype)
            tmp_batch_size = batch_size * num_videos_per_prompt

            if window_ref_video.shape[-3] == 16:
                ref_video_latents = window_ref_video
            else:
                ref_video_latents = self._encode_vae_video(video=window_ref_video, generator=generator)
            ref_video_latents = ref_video_latents.repeat(tmp_batch_size // ref_video_latents.shape[0], 1, 1, 1, 1)
            ref_video_latents = ( torch.cat([ref_video_latents] * 2) if do_classifier_free_guidance else ref_video_latents)
            zero_mask = torch.zeros_like(mask[:, :, :1, :, :])
            ref_video_latents_bk = ref_video_latents
            ref_video_latents = torch.cat([ref_video_latents, zero_mask], dim=-3)
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    # latent_branch_input = torch.cat([masked_video_latents + ref_video_latents_bk, mask[:, :, :1, :, :]], dim=-3)
                    
                    branch_block_samples = self.branch(
                        hidden_states=latent_video_input,
                        encoder_hidden_states=prompt_embeds,
                        branch_cond=latent_branch_input,
                        branch_ref_cond=ref_video_latents,
                    ...
                    if replace_gt:
                        ...
                        if do_classifier_free_guidance:
                            ...
                            init_mask = mask
                        # init_mask = (init_mask > 0.1) * 1.0
                        init_mask = init_mask.to(device)
                        #wya implementation
                        # bool_mask = (init_mask > 0.01).to(init_mask.dtype)
                        #hc implementation
                        fake_bool_mask = (init_mask > 0.1).to(init_mask.dtype)
                        fake_bool_mask[init_mask < 0.1] = (init_mask[init_mask < 0.1]) * 10
                        bool_mask = fake_bool_mask
                        ...
                        if mask_background:
                            latents = bool_mask * init_latents_proper + (1 - bool_mask) * latents
                        else:
                            latents = (1 - bool_mask) * init_latents_proper + bool_mask * latents
