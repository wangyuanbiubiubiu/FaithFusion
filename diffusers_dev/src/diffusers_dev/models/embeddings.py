...
class CogVideoXPatchEmbed(nn.Module):
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor, masks: torch.Tensor = None):
        r"""
        Args:
        ...
        if self.use_positional_embeddings or self.use_learned_positional_embeddings:
            ...
            if text_embeds is not None:
                # 有文本，添加完整的位置编码
                final_pos_embedding = pos_embedding
            else:
                # 无文本，只切片出图像对应的位置编码
                final_pos_embedding = pos_embedding[:, self.max_text_seq_length:]
            
            # 确保切片后的维度与embeds匹配
            if final_pos_embedding.shape[1] != embeds.shape[1]:
                 raise ValueError(
                    f"Shape mismatch for position embedding. Embeds shape: {embeds.shape}, "
                    f"Pos embedding shape: {final_pos_embedding.shape}"
                )

            embeds = embeds + final_pos_embedding

        # 4. 处理Mask并返回
        if masks is not None:
            # Process mask (逻辑不变)
            masks = (masks > 0.99) * 1.0
            masks = masks.reshape(-1, 1, height, width)
            masks = F.avg_pool2d(masks, kernel_size=self.patch_size, stride=self.patch_size)
            masks = masks.view(batch, num_frames, *masks.shape[1:])
            masks = masks.flatten(3).transpose(2, 3)
