...
from .transformers.cogvideox_transformer_3d import CogVideoXBlock_REF, CogVideoXTransformer3DModel
...
class CogvideoXBranchModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(
        ...
        self.patch_embed_ref = CogVideoXPatchEmbed(
            patch_size=patch_size,
            in_channels=in_channels * 2 + 1 if in_channels == 16 else in_channels + 1,
            embed_dim=inner_dim,
            text_embed_dim=text_embed_dim,
    ...
    def from_transformer(
        ...
        if load_weights_from_transformer:
            ...
            #add refs
            branch.patch_embed_ref.proj.weight.data=torch.nn.Parameter(conv_in_condition_weight)
            branch.patch_embed_ref.proj.bias.data=transformer.patch_embed.proj.bias
            ...
            for src_block, dst_block in zip(transformer.transformer_blocks, branch.transformer_blocks):
                src_state_dict = src_block.state_dict()
                dst_state_dict = dst_block.state_dict()

                # 2. 复制 attn1 -> attn_ref,norm1 -> norm_attn_ref
                for key in src_state_dict:
                    if "attn1." in key:
                        new_key = key.replace("attn1.", "attn_ref.")
                        assert new_key in dst_state_dict
                        dst_state_dict[new_key] = src_state_dict[key]
                    elif "norm1." in key:
                        new_key = key.replace("norm1.", "norm_attn_ref.")
                        assert new_key in dst_state_dict
                        dst_state_dict[new_key] = src_state_dict[key]

                        new_key = key.replace("norm1.", "norm_ref_intput.")
                        assert new_key in dst_state_dict
                        dst_state_dict[new_key] = src_state_dict[key]

                for key in src_state_dict:
                    dst_state_dict[key] = src_state_dict[key]
                dst_block.load_state_dict(dst_state_dict, strict=False)
    ...
    def forward(
        ...
        branch_ref_cond: torch.Tensor = None,
        ...
        # 2. Patch embedding 
        zeros_for_latent = torch.zeros_like(hidden_states)
        ...
        # and add ref patch embdding, hidden_states use zero
        ref_branch_cond = torch.cat([zeros_for_latent, branch_ref_cond], dim=2)
        ref_hidden_states = self.patch_embed_ref(None, ref_branch_cond)
        ref_hidden_states = self.embedding_dropout(ref_hidden_states) #dropout下，提高鲁棒性
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                attention_kwargs = {}
                attention_kwargs["ref_hidden_states"] = ref_hidden_states
                if not wo_text:
