...
class CogVideoXSpatialNorm3D(nn.Module):
    ...
    def forward(self, f: torch.Tensor, zq: torch.Tensor) -> torch.Tensor:
        ...
        if f.shape[2] > 1 and f.shape[2] % 2 == 1:
            ...
            # 1. 处理z_first的上采样：临时转Float16避免3D上采样报错
            z_first = z_first.to(dtype=torch.float16)  # BFloat16→Float16（支持3D上采样）
            z_first = F.interpolate(z_first, size=f_first_size)
            z_first = z_first.to(dtype=orig_dtype)    # 转回原始类型（BFloat16）
            
            # 2. 处理z_rest的上采样：同样临时转类型
            z_rest = z_rest.to(dtype=torch.float16)    # BFloat16→Float16
            z_rest = F.interpolate(z_rest, size=f_rest_size)
            z_rest = z_rest.to(dtype=orig_dtype)      # 转回原始类型
            
            zq = torch.cat([z_first, z_rest], dim=2)
        else:
            # 处理else分支的上采样：临时转类型
            zq = zq.to(dtype=torch.float16)            # BFloat16→Float16
            zq = F.interpolate(zq, size=f.shape[-3:])
            zq = zq.to(dtype=orig_dtype)              # 转回原始类型

        # 后续操作使用恢复后的类型（BFloat16），保持一致性
    ...
    def decode(self, z: torch.Tensor, return_dict: bool = True) -> Union[DecoderOutput, torch.Tensor]:
        ...
        Returns:
            [`~models.vae.DecoderOutput`] or `tuple`:
                ...
                returned.
        """
        if z.shape[2] == 1:  # Check if there's only 1 frame in the temporal dimension
            z = torch.cat([z, z], dim=2)  # Duplicate the frame
