...
class Attention(nn.Module):
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        quiet_attn_parameters = {"ip_adapter_masks", "ref_hidden_states", "ref_clip_weight"}
        unused_kwargs = [
...
class CogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        ref_clip_weight: Optional[float] = None,
    ) -> torch.Tensor:
        ...
        #new add ref_hidden_states
        if ref_hidden_states is not None and ref_clip_weight is not None and ref_clip_weight > 0.0:
            query_text = query[:, :, :text_seq_length]
            query_image = query[:, :, text_seq_length:]
            ...
            if image_rotary_emb is not None:
                ...
                if not attn.is_cross_attention:
                    ref_key = apply_rotary_emb(ref_key, image_rotary_emb)
        else:
            ref_key = None
            ref_value = None
        ...
        main_attn_output = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )

        attn_output_prev = None
        if prev_key is not None and prev_value is not None:
            attn_output_prev = F.scaled_dot_product_attention(
                query, prev_key, prev_value, attn_mask=None, dropout_p=0.0, is_causal=False
            )

        attn_output_ref_image = None
        if ref_key is not None and ref_value is not None:
            query_text = query[:, :, :text_seq_length]
            query_image = query[:, :, text_seq_length:]
            attn_output_ref_image = F.scaled_dot_product_attention(
                query_image, ref_key, ref_value, attn_mask=None, dropout_p=0.0, is_causal=False
            )
            
        main_attn_text = main_attn_output[:, :, :text_seq_length]
        main_attn_image = main_attn_output[:, :, text_seq_length:]

        if attn_output_prev is not None:
            prev_attn_text = attn_output_prev[:, :, :text_seq_length]
            final_text_part = main_attn_text * (1 - prev_clip_weight) + prev_attn_text * prev_clip_weight
        else:
            final_text_part = main_attn_text
        
        self_weight = 1.0
        if attn_output_prev is not None:
            self_weight = self_weight - prev_clip_weight
        
        if attn_output_ref_image is not None:
            self_weight = self_weight - ref_clip_weight

        final_image_part = main_attn_image * self_weight

        if attn_output_prev is not None:
            prev_attn_image = attn_output_prev[:, :, text_seq_length:]
            final_image_part = final_image_part + prev_attn_image * prev_clip_weight
        
        if attn_output_ref_image is not None:
            final_image_part = final_image_part + attn_output_ref_image * ref_clip_weight

        hidden_states = torch.cat([final_text_part, final_image_part], dim=2)
