...
class CogVideoXBlock_REF(nn.Module):
    r"""
    ...
    def __init__(
        ...
        # 2. ref cross Attention
        self.norm_attn_ref = CogVideoXLayerNormZero(time_embed_dim, dim, norm_elementwise_affine, norm_eps, bias=True)
        self.norm_ref_intput = CogVideoXLayerNormZero(time_embed_dim, dim, norm_elementwise_affine, norm_eps, bias=True)
        self.attn_ref_processor = CogVideoXAttnProcessor2_0() 
        self.attn_ref = Attention(
            ...
            bias=attention_bias,
            out_bias=attention_out_bias,
            processor=self.attn_ref_processor,
        )
...
#
        if attention_kwargs and "ref_hidden_states" in attention_kwargs:
            ref_hidden_states = attention_kwargs["ref_hidden_states"]
            norm_ref_hidden_states, gate_ref_msa = self.norm_attn_ref.forward_wo_text(ref_hidden_states, temb)
            attention_kwargs = attention_kwargs.copy()
            attention_kwargs["ref_hidden_states"] = norm_ref_hidden_states
            attention_kwargs["ref_clip_weight"] = 1.0
    
            norm_hidden_states, norm_encoder_hidden_states, gate_msa, enc_gate_msa = self.norm_ref_intput(
                    hidden_states, encoder_hidden_states, temb)
                    
            ref_attn_hidden_states, _ = self.attn_ref(
                hidden_states=norm_hidden_states,
                encoder_hidden_states=norm_encoder_hidden_states,
                image_rotary_emb=image_rotary_emb,
                attention_mask=attention_mask,
            ...
            # merge ref to hidden_states
            hidden_states = hidden_states + gate_msa * ref_attn_hidden_states
